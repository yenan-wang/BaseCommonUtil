 ...
 */
open class CustomBaseItemAnimator : SimpleItemAnimator() {
    ...
    override fun runPendingAnimations() {
        ...
        if (movesPending) {
            ...
            if (removalsPending) {
                ...
                view?.let {
                    ViewCompat.postOnAnimationDelayed(it, mover, removeDuration)
                }
            } else {
                mover.run()
            }
    ...
    override fun animateMove(
        ...
        val view = holder?.itemView
        view?.let {
            fromDX += it.translationX.toInt()
            fromDY += it.translationY.toInt()
        }
        resetAnimation(holder)
        val deltaX = toX - fromDX
        val deltaY = toY - fromDY
        ...
        mPendingMoves.add(MoveInfo(holder, fromDX, fromDY, toX, toY))
        return true
    }
    ...
    override fun animateChange(
        ...
        oldHolder?.let {
            val prevTranslationX = it.itemView.translationX
            val prevTranslationY = it.itemView.translationY
            val prevAlpha = it.itemView.alpha
            resetAnimation(it)
    ...
    open fun preAnimateAddImpl(holder: RecyclerView.ViewHolder?) {
        holder?.itemView?.alpha = 0f
    }

    open fun preAnimateRemoveImpl(holder: RecyclerView.ViewHolder?) {
